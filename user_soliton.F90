...
!-------------------------------------------------------------------------------
 ...
 ! impose conductor behind the left reflecting wall
        ...
        !xmin=mx0-2.
        !xmax=mx0
        Binit = sqrt(gamma0*.5*ppc0*(1+me/mi)*c**2*(me)*sigma)
				xmin=2.
				xmax=4.
        i1=iloc(int(xmin)) !convert to local index
        i2=iloc(int(xmax))
        ...
        if(i1 .ne. i2) then 
				 do  j=1,my
           bx(i1:i2,j,:)=0.!binit*cos(btheta) 
           bz(i1:i2,j,:)=0.!binit*sin(btheta)*sin(bphi)
           by(i1:i2,j,:)=Binit*exp(-(jglob(j) - 0.65*my0)*(jglob(j)-0.65*my0)/2./64./64.)*exp(-(c*lap -200.)*(c*lap-200.)/2./80./80.)&
           *COS(1.25*c*lap/c_omp)+Binit*exp(-(jglob(j) - 0.35*my0)*(jglob(j)-0.35*my0)/2./64./64.)*exp(-(c*lap-200.)*(c*lap-200.)/2./80./80.)*COS(1.25*c*lap/c_omp)!binit*sin(btheta)*cos(bphi)
           ex(i1:i2,j,:)=0.				
           ey(i1:i2,j,:)=0.!(-beta)*bz(i1:i2,:,:) 
           ez(i1:i2,j,:)=-Binit*exp(-(jglob(j) - 0.65*my0)*(jglob(j)-0.65*my0)/2./64./64.)*exp(-(c*lap -200.)*(c*lap-200.)/2./80./80.)&
           *SIN(1.25*c*lap/c_omp)-Binit*exp(-(jglob(j) - 0.35*my0)*(jglob(j)-0.35*my0)/2./64./64.)*exp(-(c*lap-200.)*(c*lap-200.)/2./80./80.)*SIN(1.25*c*lap/c_omp)!-(-beta)*by(i1:i2,:,:)    
         enddo
				endif
